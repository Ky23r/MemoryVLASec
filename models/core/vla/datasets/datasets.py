...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Offline predict_action consumes the raw PIL image and performs the
        # model's vision transform itself. Training still preprocesses here.
        pixel_values = self.image_transform(img) if self.preprocess_image else None

        # Add future actions to batch
        action = torch.tensor(action, dtype=torch.float32)
        if action.ndim == 1:
            action = action.unsqueeze(0)
        action_mask = torch.ones(action.shape[0], dtype=torch.bool)
